...
import (
	...
	"encoding/json"
	"fmt"
	"io/ioutil" // Importe o pacote ioutil
	"os"
	"os/exec"
	"strings"
)
...
func ProcessOutput(output string) {
	...
	// Crie um PackageData com a chave raiz "Package"
	packageData := PackageData{Package: make(map[string]PackageInfo)}

	// Preencha o PackageData com as informações coletadas
	for _, p := range packageInfos {
		packageData.Package[p.Name] = p
	}

	// Codifique o PackageData como JSON
	outputJSON, err := json.MarshalIndent(packageData, "", "    ")
	if err != nil {
		fmt.Printf("Erro ao codificar JSON: %v\n", err)
		return
	}

	// Escreva o JSON no arquivo de saída
