...
func searchFile(path string, config Config, wg *sync.WaitGroup, resultChan chan<- string) {
	defer wg.Done()
...
func worker(id int, jobs <-chan string, config Config, wg *sync.WaitGroup, resultChan chan<- string) {
	defer wg.Done()
	for path := range jobs {
		fmt.Printf("Worker %d processando: %s\n", id, path)
		searchFile(path, config, wg, resultChan)
	}
}
...
func main() {
	...
	for i := 0; i < maxWorkers; i++ {
		wg.Add(1)
		go worker(i, jobs, config, &wg, resultChan)
	}

	// Envia o diretório raiz para os workers
	go func() {
		jobs <- config.RootDir
		close(jobs) // Fecha o canal de jobs após enviar o diretório raiz
	}()
