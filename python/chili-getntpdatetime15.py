...
class TimeUpdater(Gtk.Window):
    def __init__(self):
        ...
        self.date_value.set_markup(
            '<span size="xx-large">' + self.get_current_date() + "</span>"
        )
        ...
        self.time_value.set_markup(
            '<span size="xx-large">' + self.get_current_time() + "</span>"
        )
        self.time_box.pack_start(self.time_value, False, False, 0)
        vbox.pack_start(self.time_box, False, False, 10)

        # SpinButtons para configurar manualmente a hora (se necessário)
        ...
        # Caixa de busca e combobox para timezones
    ...
    def update_time(self, widget):
        ...
        if selected_zone:
            ...
            if self.run_as_root(commands):
                print(
                    f"Data e hora sincronizadas com sucesso para a zona: {selected_zone}."
                )
            else:
                print(f"Falha ao sincronizar data/hora para a zona: {selected_zone}.")

        # Obtém a data e hora atual após a atualização
        now = datetime.now()

        # Atualiza os labels de data e hora
        self.time_value.set_markup(
            '<span size="xx-large">' + now.strftime("%H:%M:%S") + "</span>"
        )
        self.date_value.set_markup(
            '<span size="xx-large">' + now.strftime("%d/%m/%Y") + "</span>"
        )

        # Atualiza o calendário
        self.calendar.select_month(now.month - 1, now.year)  # Mês é 0-indexado
        self.calendar.select_day(now.day)

        # Atualiza os spinbuttons
        ...
        # A chamada queue_draw() deve forçar o redrawn, mas para garantir que a interface
        # reflita as mudanças corretamente, vamos garantir que a hora exibida seja atualizada
        # imediatamente após a atualização da interface.
        self.time_value.set_text(now.strftime("%H:%M:%S"))
        self.date_value.set_text(now.strftime("%d/%m/%Y"))
    ...
    def set_current_zoneinfo(self):
        ...
        for i in range(active_index):
            if (
                self.zone_info_combobox.get_model().get_value(
