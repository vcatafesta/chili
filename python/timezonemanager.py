...
class TimezoneManager:
    ...
    def check_root(self):
        if os.geteuid() != 0:
            print(self.colorize("Este script precisa ser executado com privilégios de superusuário (root).", "red"))
            sys.exit(1)
    ...
    def normalize_timezone_input(self, input_str):
        ...
        print(self.colorize(f"Use '{script_name} -l' para listar os timezones disponíveis.", "yellow"))
        sys.exit(1)
    ...
    def show_usage(self):
        ...
        print("\nOpções:")
        print(self.colorize("    -l, --list          # Lista todos os timezones disponíveis", "blue"))
        print(self.colorize("    -c, --config        # Exibe configurações atuais", "blue"))
        print(self.colorize("    -n, --nocolor       # Desativa cores na saída", "blue"))
        print(self.colorize("    -h, --help          # Mostra esta mensagem de ajuda", "blue"))
        print("\nExemplos de uso:")
    ...
    def main(self):
        ...
        if "-n" in args or "--nocolor" in args or "nocolor" in args:
            self.use_color = False
            args = [arg for arg in args if arg not in ("-n", "--nocolor")]

        if "-h" in args or "--help" in args or "help" in args:
            self.show_usage()
            sys.exit(0)

        if "-c" in args or "--config" in args or "config" in args:
            subprocess.run(["timedatectl"], check=True)
            sys.exit(0)

        if "-l" in args or "--list-timezones" in args or "list-timezones" in args:
            self.list_timezones()
            sys.exit(0)
