...
Local=$(zenity --list --radiolist --title "$Titulo" --height 398 \
	--text "Informe em qual local das imagens se \nsituará a marca d'água" \
...
for Arq in $NAUTILUS_SCRIPT_SELECTED_FILE_PATHS; do
	...
	echo $((++i * 100 / $TotArqs))
	echo "# Colocando marca d'agua em $(basename $Arq)"
	Arq=$(sed 's/ /\\ /g' <<<"$Arq") # Escapa espaços (por causa do eval)
	eval composite "$MarDag" -watermark $Brilho "$Local" "$Arq" "$Arq"
done | zenity --progress --title "$Titulo" --auto-close --auto-kill
