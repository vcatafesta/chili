...
# [Mandelbrot Set]
  ...
  m0()
  {
    d=\●;for((N=30,I=30,P=10**8,Q=P/100,X=320*Q/(COLUMNS-1),Y=210*Q/(LINES-2),v=-220*Q,y=-105*Q;y<105*Q;y+=Y));{ for((x=v;x<P;x+=X));{ for((a=b=i=k=c=0;a*a+b*b<2*P*P&&i++<I;a=((c=a)*a-b*b)/P+x,b=2*c*b/P+y));{ :;};(((j=(i<I?i%16:0)+N)>N+7?k=1,j-=8:0));printf "\e[$k;${j}m$d";};};printf \\e[0m
  }
...
# l=50  #  346   46
  ...
  m1()
  {
    for (( N=30 , I=99 , P=10**8 , Q=P/100 , X=320*Q/c , Y=210*Q/l , v=-220*Q , y=-105*Q ; y<105*Q ; y+=Y ))
    {
      for (( x=v ; x<P ; x+=X ))
      {
        for (( a=b=i=k=c=0 ; a*a+b*b<2*P*P && i++<I ; a=((c=a)*a-b*b)/P+x , b=2*c*b/P+y ))
        { :;}
...
# [Life Game]
  ...
  lo()
  {
    ...
    echo -ne "\E#8";b=$[X=`tput cols`,Y=`tput lines`,d=1,a=X/2,Y/2];while case $d in 0)((a=a<2?X:a-1));;1)((b=b<2?Y:b-1));;2)((a=a==X?1:a+1));;3)((b=b==Y?1:b+1));; esac;do((c=b+a*X));v=${k[c]:- };[ $v. = @. ]&&{((d=d>2?0:d+1));k[c]="";}||{(( d=d<1?3:d-1));k[c]=@;};echo -ne "\E[$b;${a}H$v";done
  }
  ...
  l0()
  {
    printf "\e#8";b=$[X=COLUMNS,Y=LINES,d=1,a=X/2,Y/2];while case $d in 0)((a=a<2?X:a-1));;1)((b=b<2?Y:b-1));;2)((a=a==X?1:a+1));;3)((b=b==Y?1:b+1));;esac;do((c=b+a*X));v=${k[c]:- };[[ $v == @ ]]&&{((d=d>2?0:d+1));k[c]='';}||{((d=d<1?3:d-1));k[c]=@;};printf "\e[$b;${a}H$v";done
  }
